...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 pt-16 bg-white">
      <div className="absolute inset-0 grid-bg opacity-50 pointer-events-none" />

      <div className="relative w-full max-w-md">
        <Link href="/" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 transition-colors mb-8">
        ...
        >
          ...
          <h1 className="font-display font-black text-[clamp(40px,6vw,68px)] uppercase leading-[0.9] mb-3 text-slate-900">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-card">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <button
                    ...
                    className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${
                      form.remember ? "bg-primary border-primary" : "border-slate-300 bg-white"
                    }`}
                  >
                ...
                <button type="button" className="text-xs text-primary hover:text-red-700 font-semibold">FORGOT PASSWORD?</button>
              </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            NOT A MEMBER YET?{" "}
            <Link href="/signup" className="text-primary hover:text-red-700 font-semibold">Sign Up</Link>
          </p>
        </motion.div>
      </div>
    </div>
